...
type UserQuery struct {
	...
	withBillingEntity         *BillingEntityQuery
...
// QueryBillingEntity chains the current query on the "billing_entity" edge.
func (_q *UserQuery) QueryBillingEntity() *BillingEntityQuery {
	query := (&BillingEntityClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBillingEntity:         _q.withBillingEntity.Clone(),
...
// WithBillingEntity tells the query-builder to eager-load the nodes that are connected to
// the "billing_entity" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBillingEntity(opts ...func(*BillingEntityQuery)) *UserQuery {
	query := (&BillingEntityClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBillingEntity = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBillingEntity; query != nil {
		if err := _q.loadBillingEntity(ctx, query, nodes, nil,
			func(n *User, e *BillingEntity) { n.Edges.BillingEntity = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadBillingEntity(ctx context.Context, query *BillingEntityQuery, nodes []*User, init func(*User), assign func(*User, *BillingEntity)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*User)
	for i := range nodes {
		if nodes[i].BillingEntityID == nil {
			continue
		}
		fk := *nodes[i].BillingEntityID
	...
	query.Where(billingentity.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "billing_entity_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withBillingEntity != nil {
			_spec.Node.AddColumnOnce(user.FieldBillingEntityID)
		}
	}
