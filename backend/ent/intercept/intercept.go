...
// The RoutingStrategyFunc type is an adapter to allow the use of ordinary function as a Querier.
type RoutingStrategyFunc func(context.Context, *ent.RoutingStrategyQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f RoutingStrategyFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.RoutingStrategyQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.RoutingStrategyQuery", q)
}

// The TraverseRoutingStrategy type is an adapter to allow the use of ordinary function as Traverser.
type TraverseRoutingStrategy func(context.Context, *ent.RoutingStrategyQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseRoutingStrategy) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseRoutingStrategy) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.RoutingStrategyQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.RoutingStrategyQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.RoutingStrategyQuery:
		return &query[*ent.RoutingStrategyQuery, predicate.RoutingStrategy, routingstrategy.OrderOption]{typ: ent.TypeRoutingStrategy, tq: q}, nil
