...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	modelRPMRuleRepository := repository.NewModelRPMRuleRepository(db)
	modelRPMRuleResolver := service.NewModelRPMRuleResolver(modelRPMRuleRepository)
	modelRPMCache := repository.NewModelRPMCache(redisClient)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, modelRPMRuleResolver, modelRPMCache)
	...
	accountHealthCache := repository.NewAccountHealthCache(redisClient)
	accountHealthService := service.NewAccountHealthService(accountHealthCache, configConfig, settingService)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator, accountHealthService)
	...
	routingStrategyRepository := repository.NewRoutingStrategyRepository(client)
	routingStrategyService := service.NewRoutingStrategyService(routingStrategyRepository)
	...
	announcementAudienceRepository := repository.NewAnnouncementAudienceRepository(client)
	announcementService := service.NewAnnouncementService(announcementRepository, announcementReadRepository, userRepository, userSubscriptionRepository, announcementAudienceRepository)
	...
	routingStrategyHandler := admin.NewRoutingStrategyHandler(routingStrategyService)
	modelRPMRuleService := service.NewModelRPMRuleService(modelRPMRuleRepository, modelRPMRuleResolver, groupRepository, userRepository)
	modelRPMRuleHandler := admin.NewModelRPMRuleHandler(modelRPMRuleService)
	...
	accountTTFTCache := repository.NewAccountTTFTCache(redisClient)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, accountTTFTCache, compositeTokenCacheInvalidator, grokQuotaService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, larkService, opsService)
	billingEntityService := service.NewBillingEntityService(client)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, billingEntityService)
	...
	userAccessTokenService := service.NewUserAccessTokenService(userAccessTokenRepository, userService)
	userAccessTokenHandler := admin.NewUserAccessTokenHandler(userAccessTokenService, userService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, routingStrategyHandler, modelRPMRuleHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, userAccessTokenHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	handlerUserAccessTokenHandler := handler.NewUserAccessTokenHandler(userAccessTokenService)
	openBalanceHandler := handler.NewOpenBalanceHandler(billingCacheService, userService, userAccessTokenService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerUserAccessTokenHandler, openBalanceHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	userAccessTokenAuthMiddleware := middleware.NewUserAccessTokenAuthMiddleware(userAccessTokenService, apiKeyService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, userAccessTokenAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
