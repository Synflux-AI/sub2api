...
func TestOpenAIGatewayServiceRecordUsage_PrefersClientRequestIDOverUpstreamRequestID(t *testing.T) {
	...
	ctx = context.WithValue(ctx, ctxkey.TraceID, "trace-openai-789")
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
	...
	require.Equal(t, "trace-openai-789", *usageRepo.lastLog.TraceID)
}
...
func TestOpenAIGatewayServiceRecordUsage_OutputImageSizeWinsBeforeBillingAndPersistence(t *testing.T) {
	...
	require.NotNil(t, usageRepo.lastLog.ImageResponseFormat)
	require.Equal(t, "url", *usageRepo.lastLog.ImageResponseFormat)
