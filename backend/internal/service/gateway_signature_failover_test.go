...
func TestShouldFailoverSignatureError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "non-signature error -> no failover (custom keywords moved to error handling rules)",
			account:  apiKeyAccount,
			settings: &RectifierSettings{Enabled: true, APIKeySignatureFailoverEnabled: true},
		...
		{
			name:     "deserialize error -> no failover (custom keywords moved to error handling rules)",
			account:  apiKeyAccount,
			settings: &RectifierSettings{Enabled: true, APIKeySignatureFailoverEnabled: true},
