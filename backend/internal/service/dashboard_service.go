...
func (s *DashboardService) GetUsageTrendByModelWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, upstreamModelMismatch *bool) ([]usagestats.TrendModelDataPoint, error) {
	trend, err := s.usageRepo.GetUsageTrendByModelWithFilters(ctx, startTime, endTime, granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, upstreamModelMismatch)
	if err != nil {
		return nil, fmt.Errorf("get usage trend by model with filters: %w", err)
	}
	return trend, nil
}
...
func (s *DashboardService) GetUserUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string, limit int, sortBy string, filters usagestats.UsageLogFilters) ([]usagestats.UserUsageTrendPoint, error) {
	trend, err := s.usageRepo.GetUserUsageTrend(ctx, startTime, endTime, granularity, limit, sortBy, filters)
	if err != nil {
