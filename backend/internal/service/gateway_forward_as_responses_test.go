...
func TestHandleResponsesBufferedStreamingResponse_RestoresNamespaceTool(t *testing.T) {
	...
	svc := &GatewayService{}
	_, err := svc.handleResponsesBufferedStreamingResponse(resp, c, "claude-fable-5", "claude-fable-5", nil, time.Now(), namespaceToolMapping(), false)
	require.NoError(t, err)
...
func TestHandleResponsesBufferedStreamingResponse_ToolArgumentsAreValidJSON(t *testing.T) {
	...
	_, err := (&GatewayService{}).handleResponsesBufferedStreamingResponse(resp, c, "claude-fable-5", "claude-fable-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{}, false)
	require.NoError(t, err)
...
func TestHandleResponsesStreamingResponse_RestoresNamespaceTool(t *testing.T) {
	...
	svc := &GatewayService{}
	_, err := svc.handleResponsesStreamingResponse(resp, c, "claude-fable-5", "claude-fable-5", nil, time.Now(), namespaceToolMapping(), false)
	require.NoError(t, err)
...
func TestHandleResponsesBufferedStreamingResponse_PreservesMessageStartCacheUsage(t *testing.T) {
	...
	svc := &GatewayService{}
	result, err := svc.handleResponsesBufferedStreamingResponse(resp, c, "claude-sonnet-4.5", "claude-sonnet-4.5", nil, time.Now(), apicompat.ResponsesClientToolMapping{}, false)
	require.NoError(t, err)
...
func TestHandleResponsesStreamingResponse_PreservesMessageStartCacheUsage(t *testing.T) {
	...
	svc := &GatewayService{}
	result, err := svc.handleResponsesStreamingResponse(resp, c, "claude-sonnet-4.5", "claude-sonnet-4.5", nil, time.Now(), apicompat.ResponsesClientToolMapping{}, false)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestOpus55ResponsesSignedThinkingBufferedAndStreamed(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		var err error
		if stream {
			_, err = svc.handleResponsesStreamingResponse(resp, c, "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{}, false)
		} else {
			_, err = svc.handleResponsesBufferedStreamingResponse(resp, c, "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{}, false)
		}
		require.NoError(t, err)
