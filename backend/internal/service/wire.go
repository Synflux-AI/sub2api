...
func ProvideConcurrencyService(cache ConcurrencyCache, accountRepo AccountRepository, cfg *config.Config) *ConcurrencyService {
	...
	if err := svc.CleanupStaleProcessSlots(context.Background()); err != nil {
		logger.LegacyPrintf("service.concurrency", "Warning: startup cleanup expired concurrency slots failed: %v", err)
	}
	if cfg != nil {
...
func ProvideOpsAlertEvaluatorService(
	...
	svc := NewOpsAlertEvaluatorService(opsService, opsRepo, emailService, larkService, redisClient, cfg, proxyRepo)
	svc.Start()
	return svc
}
...
// ProvidePaymentService creates PaymentService and attaches notification email and Lark delivery.
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, larkService *LarkService, opsService *OpsService) *PaymentService {
	svc := NewPaymentService(entClient, registry, loadBalancer, redeemService, subscriptionSvc, configService, userRepo, groupRepo, affiliateService)
	svc.SetNotificationEmailService(notificationEmailService)
	svc.SetLarkService(larkService, opsService)
	return svc
}
