...
const (
	openAIImageResponseFormatURL     = "url"
	openAIImageResponseFormatB64JSON = "b64_json"
)
...
func classifyOpenAIImagesCapability(req *OpenAIImagesRequest) OpenAIImagesCapability {
	...
	if req.ResponseFormat != "" && req.ResponseFormat != openAIImageResponseFormatB64JSON {
		return OpenAIImagesCapabilityNative
	}
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if resp.StatusCode >= 400 {
		...
		builtinWillFailover := s.shouldFailoverOpenAIUpstreamResponse(account, resp.StatusCode, upstreamMsg, respBody)
	...
	if parsed.Stream && isEventStreamResponse(resp.Header) {
		streamUsage, streamCount, streamSizes, imageResponseFormat, ttft, err := s.handleOpenAIImagesStreamingResponse(resp, c, startTime, nil)
		if err != nil {
			if streamCount > 0 {
		...
		nonStreamUsage, nonStreamCount, nonStreamSizes, imageResponseFormat, err := s.handleOpenAIImagesNonStreamingResponse(
			upstreamCtx, resp, c, account, safeUpstreamURL(upstreamReq.URL.String()), parsed)
		if err != nil {
			return nil, err
		}
...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(
	...
	if err != nil {
		return OpenAIUsage{}, 0, nil, "", s.handleOpenAIUpstreamResponseBodyReadError(
			ctx, c, account, err, false, upstreamURL)
	}
	...
	return usage, counter.Count(), counter.Sizes(), counter.ResponseFormat(), nil
}
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponse(
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), imageCounter.ResponseFormat(), firstTokenMs, err
			}
		}
		flushSSEEvent()
		finalizeFallbackBody()
		return usage, imageCounter.Count(), imageCounter.Sizes(), imageCounter.ResponseFormat(), firstTokenMs, finish()
	}
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), imageCounter.ResponseFormat(), firstTokenMs, ev.err
			}
			processLine(ev.line)
		case <-intervalCh:
			...
			if clientDisconnected {
				return usage, imageCounter.Count(), imageCounter.Sizes(), imageCounter.ResponseFormat(), firstTokenMs, fmt.Errorf("image stream incomplete after timeout")
			}
			...
			return usage, imageCounter.Count(), imageCounter.Sizes(), imageCounter.ResponseFormat(), firstTokenMs, fmt.Errorf("image stream data interval timeout")
		case <-keepaliveCh:
...
func extractOpenAIImagesBillableCountFromJSONBytes(body []byte) int {
	...
	if gjson.GetBytes(body, openAIImageResponseFormatB64JSON).Exists() || gjson.GetBytes(body, openAIImageResponseFormatURL).Exists() {
		return 1
	}
	return 0
}
