...
//   - consultRules 为 true 时先问错误处理规则，命中则按规则的结论返回；
//   - 其余一律返回 *UpstreamFailoverError，由 handler 的 failover 循环换号。
//
// 本函数不写响应：响应归 handler 所有（换号，或耗尽后按端点格式渲染错误）。
func (s *GeminiMessagesCompatService) handleUpstreamTransportError(ctx context.Context, c *gin.Context, account *Account, err error, consultRules bool) error {
