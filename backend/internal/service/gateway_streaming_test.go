...
func TestParseSSEUsage_DeltaDoesNotOverwriteStartValues(t *testing.T) {
	...
	// 先处理 message_start
	svc.parseSSEUsage(`{"type":"message_start","message":{"usage":{"input_tokens":100}}}`, usage, false)
	require.Equal(t, 100, usage.InputTokens)

	// 再处理 message_delta（output_tokens > 0, input_tokens = 0）
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"output_tokens":50}}`, usage, false)
...
func TestParseSSEUsage_DeltaOverwritesWithNonZero(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"input_tokens":200,"output_tokens":100,"cache_creation_input_tokens":30,"cache_read_input_tokens":60}}`, usage, false)
...
func TestParseSSEUsage_DeltaAuthoritativelyUpdatesCacheCreationBreakdown(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"message_start","message":{"usage":{"cache_creation_input_tokens":463184,"cache_creation":{"ephemeral_5m_input_tokens":0,"ephemeral_1h_input_tokens":463184}}}}`, usage, false)
	...
	svc.parseSSEUsage(`{"type":"message_delta","usage":{"cache_creation_input_tokens":463184,"cache_creation":{"ephemeral_5m_input_tokens":463184,"ephemeral_1h_input_tokens":0}}}`, usage, false)
	require.Equal(t, 463184, usage.CacheCreationInputTokens)
...
func TestParseSSEUsage_InvalidJSON(t *testing.T) {
	...
	// 无效 JSON 不应 panic
	svc.parseSSEUsage("not json", usage, false)
...
func TestParseSSEUsage_UnknownType(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"content_block_delta","delta":{"text":"hello"}}`, usage, false)
	require.Equal(t, 0, usage.InputTokens)
	require.Equal(t, 0, usage.OutputTokens)
}
...
func TestParseSSEUsage_DoneEvent(t *testing.T) {
	...
	// [DONE] 事件不应影响 usage
	svc.parseSSEUsage("[DONE]", usage, false)
	require.Equal(t, 0, usage.InputTokens)
}
