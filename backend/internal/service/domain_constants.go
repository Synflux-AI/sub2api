...
const (
	...
	// SettingKeySchedulingTTFTMonitorEnabled 首 Token 时延巡检：按账号×分组×实际模型聚合 TTFT 并写快照供面板展示，不影响调度
	SettingKeySchedulingTTFTMonitorEnabled = "scheduling_ttft_monitor_enabled"
	// SettingKeySchedulingTTFTDegradeEnabled 相对同分组、同实际模型基线明显变慢的账号是否扣健康分（进而降到候选池）
