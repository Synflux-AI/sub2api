...
func TestPrepareUsageLogInsert_PersistsImageSizeMetadata(t *testing.T) {
	...
	require.Equal(t, sql.NullString{String: responseFormat, Valid: true}, prepared.args[len(prepared.args)-3])
}
...
func TestUsageLogRepositoryAggregatesApplyOutputTokensAndStream(t *testing.T) {
	...
	outputTokens := 0
	stream := false
	filters := usagestats.UsageLogFilters{OutputTokens: &outputTokens, Stream: &stream}

	t.Run("total trend", func(t *testing.T) {
		db, mock := newSQLMock(t)
		repo := &usageLogRepository{sql: db}
		...
		rows, err := repo.GetUsageTrendWithUsageFilters(context.Background(), start, end, "day", filters)
		require.NoError(t, err)
	...
	t.Run("model trend", func(t *testing.T) {
		...
		mock.ExpectQuery(`AND output_tokens = \$3 AND stream = \$4`).
			WithArgs(start, end, outputTokens, stream).
			WillReturnRows(sqlmock.NewRows([]string{"date", "model", "requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens", "total_tokens", "cost", "actual_cost"}))

		rows, err := repo.GetUsageTrendByModelWithUsageFilters(context.Background(), start, end, "day", filters)
		require.NoError(t, err)
		require.Empty(t, rows)
		require.NoError(t, mock.ExpectationsWereMet())
	})
	...
	t.Run("user trend", func(t *testing.T) {
		db, mock := newSQLMock(t)
		repo := &usageLogRepository{sql: db}
		mock.ExpectQuery(`(?s)WITH filtered AS .*output_tokens = \$3.*stream = \$4.*LIMIT \$5`).
			WithArgs(start, end, outputTokens, stream, 8).
			WillReturnRows(sqlmock.NewRows([]string{"date", "user_id", "key", "label", "email", "username", "notes", "requests", "tokens", "cost", "actual_cost"}))
...
func TestUsageLogRepositoryGetUsageTrendWithUsageFiltersIncludesLatencyWithoutFilters(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}

	start := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)
	mock.ExpectQuery("percentile_cont\\(0.50\\).*FROM usage_logs").
		WithArgs(start, end).
		WillReturnRows(sqlmock.NewRows([]string{
			"date", "requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
			"total_tokens", "cost", "actual_cost",
			"duration_p50_ms", "duration_p95_ms", "duration_p99_ms", "duration_sample_count",
			"ttft_p50_ms", "ttft_p95_ms", "ttft_p99_ms", "ttft_sample_count",
		}).AddRow("2025-01-01", int64(2), int64(10), int64(20), int64(0), int64(0), int64(30), 0.1, 0.08,
			120.0, 200.0, 220.0, int64(2), 40.0, 70.0, 90.0, int64(2)))
	...
	require.Equal(t, 40.0, *trend[0].TTFT.P50Ms)
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestUsageLogRepositoryGetUsageTrendWithUsageFiltersLatencyAndScheduledAccount(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	mock.ExpectQuery("WHERE created_at >= \\$1 AND created_at < \\$2\\s+AND user_id = \\$3 AND account_id = \\$4 AND COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$5").
		WithArgs(start, end, int64(7), int64(23), "gpt-5").
		WillReturnRows(sqlmock.NewRows([]string{
			"date", "requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
			"total_tokens", "cost", "actual_cost",
			"duration_p50_ms", "duration_p95_ms", "duration_p99_ms", "duration_sample_count",
			...
			120.0, 200.0, 220.0, int64(2), nil, nil, nil, int64(0)))

	trend, err := repo.GetUsageTrendWithUsageFilters(context.Background(), start, end, "day", filters)
	require.NoError(t, err)
	require.Len(t, trend, 1)
	require.Equal(t, int64(2), trend[0].Duration.SampleCount)
	require.Equal(t, 120.0, *trend[0].Duration.P50Ms)
	require.Equal(t, int64(0), trend[0].TTFT.SampleCount)
	require.Nil(t, trend[0].TTFT.P50Ms)
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$1.*GROUP BY GROUPING SETS").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 20.0, 20.0, 20.0, 20.0, int64(1), 5.0, 5.0, 5.0, int64(1)).
			AddRow(0, 1, "/v1/responses", nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 20.0, 20.0, 20.0, 20.0, int64(1), 5.0, 5.0, 5.0, int64(1)).
			AddRow(1, 0, nil, "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 20.0, 20.0, 20.0, 20.0, int64(1), 5.0, 5.0, 5.0, int64(1)).
			AddRow(0, 0, "/v1/responses", "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 1.2, 20.0, 20.0, 20.0, 20.0, int64(1), 5.0, 5.0, 5.0, int64(1)))
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"duration_p50_ms", "duration_p95_ms", "duration_p99_ms", "duration_sample_count",
			"ttft_p50_ms", "ttft_p95_ms", "ttft_p99_ms", "ttft_sample_count",
		}).AddRow(1, 1, nil, nil, int64(50), int64(1000), int64(2000), int64(60), int64(40), 15.0, 12.5, 11.0, 100.0, 100.0, 180.0, 200.0, int64(50), 25.0, 60.0, 75.0, int64(40)))
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("service_tier_is_scanned", func(t *testing.T) {
		...
		require.Equal(t, "trace-visible-123", *log.TraceID)
	})
