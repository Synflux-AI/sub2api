...
func TestMigrationsRunner_IsIdempotent_AndSchemaIsUpToDate(t *testing.T) {
	...
	requireColumn(t, tx, "usage_logs", "image_response_format", "character varying", 16, true)
...
`, usageLogsUpstreamModelMismatchIndex).Scan(&mismatchIndexDef))
	...
	requireConstraintDefinitionContains(
		...
		tx,
		"usage_logs",
		"usage_logs_image_response_format_check",
	...
	requireIndex(t, tx, "ops_system_logs", "idx_ops_system_logs_trace_id")
