...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if filters.APIKeyID > 0 {
		...
		conditions, args = appendUsageLogIDWhereCondition(conditions, args, "api_key_id", 0, filters.APIKeyIDs)
	}
	...
	if traceID := strings.TrimSpace(filters.TraceID); traceID != "" {
		conditions = append(conditions, fmt.Sprintf("trace_id = $%d", len(args)+1))
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	return filters.UserID == 0 && len(filters.UserIDs) == 0 && filters.APIKeyID == 0 && len(filters.APIKeyIDs) == 0 && filters.AccountID == 0 && len(filters.AccountIDs) == 0
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		imageResponseFormat       sql.NullString
	...
	if imageResponseFormat.Valid {
		log.ImageResponseFormat = &imageResponseFormat.String
	}
	if groupID.Valid {
		value := groupID.Int64
		log.GroupID = &value
	...
	// 分阶段耗时：NULL 保持 nil（未测到），0 照原样保留（测到但不足 1ms）
