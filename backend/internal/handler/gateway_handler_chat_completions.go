...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	fs := NewFailoverState(h.maxAccountSwitches, false).WithUpstreamAttemptBudget(h.maxUpstreamAttempts)
	if groupPlatform == service.PlatformGemini {
		fs = NewFailoverState(h.maxAccountSwitchesGemini, false).WithUpstreamAttemptBudget(h.maxUpstreamAttempts)
	}
	...
	for {
		...
		// routingStart 不随 failover 重置：重试等待与切号成本都算「路由」，
		// 这才能把「我们自己在打转」和「上游慢」分开。
