      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            >
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.disableTempUnsched.title") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.disableTempUnsched.description") }}
              </p>
            </div>
            <div class="space-y-5 p-6">
              <div
                v-if="disableTempUnschedLoading"
                class="flex items-center gap-2 text-gray-500"
              >
                <div
                  class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"
              ...
              <template v-else>
                <div class="flex items-center justify-between">
                  <div>
                    <label class="font-medium text-gray-900 dark:text-white">{{
                      t("admin.settings.disableTempUnsched.enabled")
                    }}</label>
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                      {{ t("admin.settings.disableTempUnsched.enabledHint") }}
                    </p>
                  </div>
                  <Toggle v-model="disableTempUnschedForm.enabled" />
                </div>
                ...
                >
                  <button
                    type="button"
                    @click="saveDisableTempUnschedSettings"
                    :disabled="disableTempUnschedSaving"
                    class="btn btn-primary btn-sm"
                  >
                    <svg
                      v-if="disableTempUnschedSaving"
                      class="mr-1 h-4 w-4 animate-spin"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                  ...
                  >
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        :disabled="
                          index === errorHandlingRuleForm.rules.length - 1
                        "
                        class="btn btn-ghost btn-xs h-7 w-7 p-0"
                        :title="t('admin.settings.errorHandlingRule.moveDown')"
                        :aria-label="
                          t('admin.settings.errorHandlingRule.moveDown')
                        "
                        @click="moveErrorHandlingRule(index, 1)"
                      >
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="failover">
                            {{
                              t(
                                "admin.settings.errorHandlingRule.actionFailover",
                              )
                            }}
                          </option>
                          <option value="passthrough">
                            {{
                              t(
                                "admin.settings.errorHandlingRule.actionPassthrough",
                              )
                            }}
                          </option>
                        </select>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        :placeholder="
                          t(
                            'admin.settings.errorHandlingRule.keywordsPlaceholder',
                          )
                        "
                    ...
                    <div v-if="rule.action === 'retry'">
                      ...
                      <input
                        ...
                        :placeholder="
                          t(
                            'admin.settings.errorHandlingRule.retryCountPlaceholder',
                          )
                        "
                        @change="
                          setRuleRetryCount(
...
// 禁止临时停止调度 状态
const disableTempUnschedLoading = ref(true);
const disableTempUnschedSaving = ref(false);
const disableTempUnschedForm = reactive({
  enabled: false,
});
...
// 禁止临时停止调度 方法
async function loadDisableTempUnschedSettings() {
  disableTempUnschedLoading.value = true;
  try {
    const settings = await adminAPI.settings.getDisableTempUnschedSettings();
    Object.assign(disableTempUnschedForm, settings);
  } catch (_error: unknown) {
    // Silent fail - settings will use defaults
  } finally {
    disableTempUnschedLoading.value = false;
  }
}

async function saveDisableTempUnschedSettings() {
  disableTempUnschedSaving.value = true;
  try {
    const updated = await adminAPI.settings.updateDisableTempUnschedSettings({
      enabled: disableTempUnschedForm.enabled,
    });
    Object.assign(disableTempUnschedForm, updated);
    appStore.showSuccess(t("admin.settings.disableTempUnsched.saved"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.settings.disableTempUnsched.saveFailed"),
      ),
    );
  } finally {
    disableTempUnschedSaving.value = false;
  }
}
...
onMounted(() => {
  ...
  loadAdminApiKey();
  loadDisableTempUnschedSettings();
