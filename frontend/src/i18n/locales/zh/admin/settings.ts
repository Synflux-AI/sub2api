export default {
    settings: {
      ...
      panelRateLimit: {
        ...
        openApiRpmHint: '面向客户的 Open API（/open/balance，按访问令牌鉴权）每个用户每分钟的请求上限；0 表示不限制。',
        perMinute: '次/分钟',
        exemptAdmin: '管理员豁免',
      ...
      errorHandlingRule: {
        ...
        maxUpstreamLatencyHint:
          '仅当本次上游耗时低于该值才命中此规则；0 = 不限制。耗时未知时视为不满足，规则不命中。计的是**本次**上游尝试的耗时，流式请求上是「拿到响应头」为止（TTFB），不是整段流的时长。用于避免对已计费的慢失败做重复扣费的重试。',
        ...
        ruleEnabled: '已启用',
        ruleDisabled: '已禁用',
        retryCountColumn: '重试次数',
        retryCountDefault: '默认（{count}）',
        noRules: '还没有配置任何规则',
        editRule: '编辑规则',
        deleteConfirmTitle: '删除规则',
        deleteConfirmMessage: '确定要删除规则「{name}」吗？删除后需点击「保存」才会生效。',
        maxRulesReached: '最多只能配置 {max} 条规则',
        statusCodes: '状态码',
        keywords: '关键字',
