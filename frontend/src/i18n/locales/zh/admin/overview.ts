export default {
    ...
    users: {
      ...
      accessToken: {
        ...
        revoke: '撤销',
        revokeTitle: '撤销访问令牌',
        revokeConfirm: '确定要撤销该用户的访问令牌吗？依赖该令牌的客户端 Open API 集成会立即失效，此操作无法撤销。',
        generateSuccess: '访问令牌已生成',
        rotateSuccess: '访问令牌已重新生成',
        revokeSuccess: '访问令牌已撤销',
        generateFailed: '生成访问令牌失败',
        rotateFailed: '重新生成访问令牌失败',
    ...
    routingStrategies: {
      ...
      disabled: '停用',
      priority: '优先级',
      priorityHint: '数值越小越先评估（首个命中的策略生效）',
      platform: '平台',
      anyPlatform: '任意平台',
      group: '分组',
      globalScope: '全局（所有分组）',
      ...
      matchAny: '满足任一条件',
      action: '动作',
      actionRestrict: '只能路由（restrict）',
      actionPrefer: '优先（prefer，可回退）',
      accountPriorityLabel: '优先级',
      accountPriorityHint: '可为每个账号指定优先级：数值越小越优先；相同数值为同一优先级，再按负载 + LRU 均衡（默认算法：优先级 + 负载 + LRU）。留空均为同级。',
      conditions: '匹配条件',
      addCondition: '添加条件',
      noConditions: '无条件（命中作用域内的所有请求）',
      conditionType: '类型',
      conditionOp: '操作符',
      conditionValue: '值',
      typeModel: '模型',
      ...
      loadFailed: '加载策略失败',
      nameRequired: '请填写名称',
      accountsRequired: '请至少选择一个目标账号',
      valueRequired: '请填写条件值',
      tester: '测试',
      testerTitle: '策略测试（dry-run）',
      testModel: '模型',
      testClient: '客户端',
      testUserAgent: 'User-Agent',
      runTest: '运行测试',
      testMatched: '命中：{name}（#{id}）',
      testNoMatch: '没有策略命中',
      testResultAction: '动作',
      testResultAccounts: '目标账号'
    },
